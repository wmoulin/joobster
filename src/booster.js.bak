...
commander
    ...
    .usage("[options] <tasks ...>")
    .option("-l --log <level>", "log level, /^(info|debug|warning|error)$/i", "error")
    .option("-d --dir <directory>", "project directory", process.cwd())
    .option("-f --files <glob files>", "fichiers à lancer (syntaxe node-glob)", "")
    .parse(process.argv);
...
tasksLoader(gulp, builderDesc, function() {
  ...
  gulpHelper.parameters.filesParameter = commander.files;
